...
module.exports.createCollection = async function (name, options) {
  if (!this.db) {
    this.errorHandler.throw(
      new Error('cannot create collection -- not connected to db!')
    )
    return
  }
...
module.exports.insert = async function (docs, options) {
  if (!this.db) {
    this.errorHandler.throw(new Error('cannot insert -- not connected to db!'))
  }

  if (!this.collection || !this.collectionName) {
    this.errorHandler.throw(new Error('cannot insert -- no collection set!'))
  }

  if (!docs) {
    this.errorHandler.throw(new Error('cannot insert -- no docs to insert!'))
  }
