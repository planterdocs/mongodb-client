...
module.exports.insert = async function (docs, options) {
  if (!this.db) {
    console.error('cannot insert -- not connected!')
    return
  }

  if (!this.collection || !this.collectionName) {
    console.error('cannot insert -- collection not set!')
    return
  }

  if (!docs) {
    console.error('cannot insert -- no documents to insert!')
    return
  }
  ...
  // try {
  //   const response = await this.collection.insert(doc, options)
